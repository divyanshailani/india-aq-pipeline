...
def plot_best_station():
    ...
    for i, h in enumerate(HORIZONS):
        ...
        df_h, features = build_v11_features(df_in, viirs_data, h)
        ...
        ax.plot(range(len(y_pred_plot)), y_pred_plot, 'r--', label='Predicted (V11 AOD)', linewidth=1.5)
    ...
    plt.suptitle(f'V11 3D Atmospheric Ensemble Forecast: {best_name}', fontsize=14, fontweight='bold')
    plt.tight_layout()
