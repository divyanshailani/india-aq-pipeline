...
def load_base_data(conn, country_code):
    ...
    try:
        ...
        for f in csv_files:
            ...
            # Hard filter for India bounds (approx 6-38 N, 68-98 E) to save massive memory
...
def build_v9_features(df, viirs, horizon):
    ...
    # Spatial Join (The Blast Radius Engine)
    ...
    # --- Micro-Physics & Momentum Injections ---
    
    # 1. PM2.5 Momentum (Velocity)
    ...
    # 3. Wind Vectorization
    # wind_direction exists, convert to radians and get U/V
    if "wind_direction" in df_v9.columns:
        # Fill missing wind_direction with 0 before math if necessary
        wd = df_v9["wind_direction"].fillna(0)
        df_v9["wind_u"] = np.cos(wd * np.pi / 180)
        df_v9["wind_v"] = np.sin(wd * np.pi / 180)
    
    # Drop rows with NaNs in required lags/rolling
...
def main():
    ...
    df, viirs = load_base_data(conn, country_code)
    df_h, features = build_v9_features(df, viirs, horizon)
    target_col = "value"
    ...
    # 2. Target Transformation (Delta Math)
    lag_col = f"pm25_lag_{horizon}"
    ...
    print("Starting Optuna study (Delta Target + Fire-Wind Interaction)...")
