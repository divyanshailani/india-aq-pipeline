...
def patch_weather_batch():
    ...
    fallback_manager = ApiFallbackManager(max_retries=3, base_backoff=2.0)
    ...
    for idx, (sid, lat, lon, min_date, max_date, count) in enumerate(stations):
        ...
        try:
            w_res = fallback_manager.request_with_fallback(w_url, params=w_params)
            a_res = fallback_manager.request_with_fallback(a_url, params=a_params)
