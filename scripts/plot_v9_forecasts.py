...
from train_v9_xgboost import load_base_data, build_v9_features, temporal_split
...
def plot_best_station():
    ...
    # We will plot for India (IN) since it has high variance and Faridabad is in IN
    df_in = load_base_data(conn, "IN")
    ...
    for i, h in enumerate(HORIZONS):
        ...
        df_h, features = build_v9_features(df_in, h)
        
        # Filter to best station
        ...
        # Load V9 XGBoost model and medians
        ...
        y_pred = [max(0, p) for p in y_pred] # PM2.5 can't be negative
        
        mae = mean_absolute_error(y_test, y_pred)
        
        ax = axes[i // 2, i % 2]
        
        # We plot the last 60 days of the test set for better visibility
        ...
        ax.plot(range(len(y_pred_plot)), y_pred_plot, 'r--', label='Predicted (V9 Native)', linewidth=1.5)
        
        ax.set_title(f'{h}-Day Direct Forecast (MAE={mae:.1f})')
        ax.set_xlabel('Days (Test Split)')
        ax.set_ylabel('PM2.5 (µg/m³)')
        ax.legend()

    plt.suptitle(f'V9 XGBoost Direct Forecast: {best_name}', fontsize=14, fontweight='bold')
    plt.tight_layout()
    
    out_dir = os.path.join(os.path.dirname(__file__), "..", "plots")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, 'forecast_horizons.png')
