...
for cc in COUNTRIES:
    ...
    # Inject healed weather
    df['future_temp'] = df['om_temperature']
    df['future_wind'] = df['om_wind_speed']
    df['future_precip'] = df['om_precipitation']
    ...
    print(f"{cc} (June 21-25) -> MAE: {mae:.2f}, NMAE: {nmae:.2f}, Acc: {acc_pct:.1f}%, MASE: {mase:.2f}, Samples: {len(y_actual)}")
